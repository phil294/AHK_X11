...
module Run
	...
	# on hotstring match.
	class Hotstrings
		@hotstrings : Array(Hotstring)
		def initialize(@end_chars, @hotstrings)
		end

		def register(runner)
			runner.x11.register_key_listener do |key_event, keysym, char|
		...
		def handle_event(runner, key_event, keysym, char)
