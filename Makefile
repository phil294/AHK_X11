...
# TODO: is libxkbcommon here really necessary? this comes from evdev branch build.sh
	OUTPUT=ahk_x11.AppImage ./linuxdeploy-x86_64.AppImage --appdir AppDir --output appimage --desktop-file ./assets/ahk_x11.desktop --icon-file ./assets/ahk_x11.png --executable ./bin/ahk_x11 --library ./libxkbcommon.so.1 --library /usr/lib/x86_64-linux-gnu/libthai.so.0 --plugin gtk
	rm -rf AppDir
...
# TODO: if this doesn't work perhaps the dynamic one needs to be linked instead / too for some reason? libxkbcommon.so.1.0.0
	shards build -Dpreview_mt --link-flags="-no-pie \
        -L'${PWD}/xdotool' \
        -L'${PWD}/libxkbcustom.a' \
        -Wl,-Bstatic -lxkbcommon -lxdo \
        -Wl,-Bdynamic -lxkbcustom -lXinerama -lXext -lXtst -lXi -lwayland-client" \
        $(BUILD_EXTRA_ARGS)
